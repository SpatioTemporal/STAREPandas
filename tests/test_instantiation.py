...
def test_points():
    ...
    stare1 = starepandas.stare_from_xy(df.Longitude, df.Latitude, level=5)
    stare2 = starepandas.stare_from_xy_df(gdf, n_workers=1, level=5)
    stare3 = starepandas.stare_from_gdf(gdf, level=5)
...
def test_polygon():
    ...
    stare = starepandas.stare_from_gdf(africa, level=5, force_ccw=True)
