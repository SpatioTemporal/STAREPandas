...
Covers each row of the §C2 sizing table plus boundary and edge cases.
"""
...
# ----- Row 1 & 2: N <= MAX → single ticket, one worker --------------------


@pytest.mark.parametrize("n", [1, 5, 9, 10, 25, 39, 40])
def test_small_batch_single_ticket(n):
    """N <= 40 produces exactly one ticket containing all URIs."""
    tickets = split_into_tickets(_uris(n), workers=4)
    assert len(tickets) == 1
    assert len(tickets[0]) == n
    assert tickets[0] == _uris(n)


# ----- Row 3: MAX < N <= W * MAX → W tickets, ceil(N/W) each --------------
...
# ----- Row 4: N > W * MAX → ceil(N/MAX) tickets of MAX --------------------
