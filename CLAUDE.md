...
10. C-1 unit tests pass (cloud.ticket_sizing + metadata + granule_timestamps
    + s3_layout + ingest_module + config_env_secret — currently 61 unit tests)
...
*Last Updated: 2026-06-07 (Path C C-4 Part A "first light" complete: the
first worker ran end-to-end in Fargate. NAT topology replaced the
no-NAT/5-interface-endpoint VPC (net idle ~$73→~$32/mo); a new
`STAREPANDAS_WORKER_SECRET` env-var config loader
...
(`starepods/worker:dev`, `sha256:bc32e21d…`). Both deferred C-3 runtime DoD
items closed. C-4 Part B (Scheduler/API/Status/Budgets) is next. See
`docs/path_c_implementation.md` §C-4 Part A for the full record.)*
