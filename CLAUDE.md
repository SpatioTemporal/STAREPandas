...
*Last Updated: 2026-06-07 (Path C C-4 COMPLETE — both parts. Part A "first
light": NAT topology replaced the no-NAT/5-interface-endpoint VPC (net idle
~$73→~$32/mo); a new `STAREPANDAS_WORKER_SECRET` env-var config loader
(`_load_config_from_default_locations` + shared `_apply_config_data`) lets
the worker read its config from the injected secret; worker image rebuilt
(`starepods/worker:dev`, `sha256:bc32e21d…`). Part B control plane:
`starepods-scheduler` + `starepods-status` Lambdas behind REST API
`starepods-api` (key + usage plan) + a tag-filtered `starepods-monthly`
Budgets alarm; pure helpers vendored in `infra/cdk/lambdas/`. Full
API→scheduler→SQS→worker run verified (job `be4e3455`). C-5 (completion
watcher) is next. See `docs/path_c_implementation.md` §C-4 for the record.)*
