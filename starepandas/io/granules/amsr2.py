...
class AMSR2(Granule):
    ...
    def __init__(self, file_path, sidecar_path=None, scans=['S1', 'S2', 'S3', 'S4', 'S5', 'S6']):
    ...
    def read_timestamps(self):
        # AMSR2 specific timestamp handling based on scan dimensions
        for scan in ['S1', 'S2', 'S3', 'S4']:
            if scan in self.scans:
                ts = self.read_timestamp_scan(scan)
                ts_len = ts.shape[0]
                # S1-S4 have 243 pixels per scan
                ts = numpy.repeat(ts, 243)
                ts = ts.reshape(ts_len, 243)
        ...
        for scan in ['S5', 'S6']:
            if scan in self.scans:
                ts = self.read_timestamp_scan(scan)
                ts_len = ts.shape[0]
                # S5-S6 have 486 pixels per scan (higher resolution)
                ts = numpy.repeat(ts, 486)
                ts = ts.reshape(ts_len, 486)
                self.timestamps[scan] = ts

    def read_data(self):
        # AMSR2 specific data reading based on channel configuration
        for scan in ['S1', 'S2', 'S3', 'S4', 'S5', 'S6']:
            if scan in self.scans:
                if self.file_type == 'hdf5':
                    # All AMSR2 scans have 2 channels
                    for i in range(2):
                        self.data[scan][f'Tc{i+1}'] = self.dataset[scan]['Tc'][:, :, i]
                else:  # netcdf4
                    for i in range(2):
                        self.data[scan][f'Tc{i+1}'] = self.dataset.groups[scan]['Tc'][:, :, i]
    ...
    def create_sidecar(self, n_workers=1, cover_res=None, out_path=None):
        """Create a STARE sidecar file for this AMSR2 granule.
