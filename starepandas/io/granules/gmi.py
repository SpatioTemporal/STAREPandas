...
class GMI(Granule):
    ...
    def read_timestamps(self):
        # GMI specific timestamp handling based on scan dimensions
        if 'S1' in self.scans:
            ts = self.read_timestamp_scan('S1')
            ts_len = ts.shape[0]
            # S1 has 221 pixels per scan
            ts = numpy.repeat(ts, 221)
    ...
    def create_sidecar(self, n_workers=1, cover_res=None, out_path=None):
        """Create a STARE sidecar file for this GMI granule.
